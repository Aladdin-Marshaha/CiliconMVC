...
public class AddressService(AddressRepository repository)
{
    ...
    public async Task<ResponseResult> GetOrCreateAddressAsync(string streetName, string postaCode, string city)
    {
        try
        {
            var result = await GetAddressAsync(streetName, postaCode, city);
            if (result.StatusCode == StatusCode.NOT_FOUND)
                result = await CreateAddressAsync(streetName, postaCode, city);
